...
Provides syntax highlighting, code navigation, and incremental parsing for editors that support tree-sitter: Neovim, Zed, Emacs 29+, and (coming) VS Code.

## Highlight queries

Standard highlight queries are included at `queries/highlights.scm` using the capture names expected by Neovim, Zed, and other tree-sitter-aware editors.
...
To use in Neovim, copy `queries/injections.scm` to your nvim-treesitter queries directory or configure it via your plugin manager.
